...
CLI Usage:
python Code/Visualization/Visualization.py --icd C00_C97 --model brms
python Code/Visualization/Visualization.py --all --model mice
python Code/Visualization/Visualization.py --all --model brms
...
def get_paths(project_root: Path, cfg: dict, model: str = "eqi_lmm") -> Dict[str, Path]:
    ...
    if model == "mice":
        result_dir = project_root / cfg["result_directories"].get(
            "eqi_lmm_mi", cfg["result_directories"]["eqi_lmm"]
        )
...
def _list_icds_for_brms(result_dir: Path, cfg: dict) -> List[str]:
    ...
    regexes = [
        ...
        re.compile(r"^(.+?)_main\.csv$", re.IGNORECASE),
    ]
    for pat in patterns:
...
def list_icds_from_results(
    result_dir: Path, cfg: dict, model: str = "eqi_lmm"
) -> List[str]:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate 4 panel images per ICD for a given model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="eqi_lmm",
        choices=["eqi_lmm", "mice", "brms"],
        help="Model: eqi_lmm (default), mice, or brms",
    )
    ...
    for icd in icds:
        ...
        if args.model == "brms":
            brms_cfg = cfg.get("brms_analysis", {}).get("results", {})
            template = brms_cfg.get(
                "filename_template", "brms_{cancer_type}_Results.csv"
            )
        ...
        for eqi, aamr, lag in SCENARIO_ORDER:
            ...
            if sub.empty:
                print(
                    f"[{icd}] no data for scenario {eqi}/{aamr}/Lag{lag}; skipping panel."
                )
                continue
...
def plot_reference_style_forest(
    df,
    eqi_period,
    aamr_period,
    lag,
    ...
    # Format title
    title = f"({scenario_label}) {cancer_name} {lag}-year Lag"
    ...
    for panel_idx, panel_label in enumerate(panel_labels):
        ...
        if panel_idx == 0:  # mostratified - 使用整体模型
            ...
            rucc_num = panel_idx
            panel_models = [
                f"RUCC{rucc_num}_EQI",
                f"RUCC{rucc_num}_EQI_Air",
                f"RUCC{rucc_num}_EQI_Water",
                f"RUCC{rucc_num}_EQI_Land",
                f"RUCC{rucc_num}_EQI_Built",
                f"RUCC{rucc_num}_EQI_Social",
            ]
        ...
        elif icd_code == "G20_G30_G12.2_F01_F03" and panel_idx == 0:
            ax.set_ylim(-10, 30)
    ...
    if output_dir:
        ...
        output_filename = (
            output_path
            / f"{icd_code}_{eqi_period}_{aamr_period}_Lag{lag}_{model_type}.png"
        )
    else:
        output_filename = (
            f"{icd_code}_{eqi_period}_{aamr_period}_Lag{lag}_{model_type}.png"
