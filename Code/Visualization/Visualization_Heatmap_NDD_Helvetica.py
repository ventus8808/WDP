...
csv_path = base_dir / "Result" / "brms_heatmap" / "NDD.csv"
...
# Create custom colormap using specified blue gradient
colors = [
    "#F5F8FA",  # 0
    "#BFDAE9",  # 2
    "#89BED9",  # 4
    ...
    "#2165AA",  # 8
    "#073162",  # 10+
    "#073162",  # 30 (same as 10 to maintain color)
]
# Map to [0, 2, 4, 6, 8, 10, 30] in a 0-30 range
positions = [0.0, 0.067, 0.133, 0.200, 0.3, 0.5, 1.0]
cmap = LinearSegmentedColormap.from_list(
    "ndd_cmap", list(zip(positions, colors)), N=256
)

# Set normalization to [0, 30]
norm = plt.Normalize(vmin=0, vmax=30)
sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])

# Define display configuration for NDD
outcomes = [
    "NDD",
    "HD",
    "PD",
...
# Add colorbar
cbar = fig.colorbar(
    sm, ax=axes, orientation="horizontal", fraction=0.08, pad=0.2, shrink=1, aspect=40
)
...
output_path = output_dir / "NDD_Helvetica.png"
