#!/usr/bin/env Rscript
# ============================================================================
# Publication-Quality Gradient Ridgeline Plots for NDD (Neurodegenerative Diseases)
# ============================================================================
...
#
# Usage:
#   Rscript RidgeLine_NDD.R --mode <mode> [options]
#
# Modes:
...
#   Rscript RidgeLine_NDD.R --mode domains_by_lag \
#     --output Result/Ridgeline/Fig_NDD_Domains_ByLag.png
#
#   # Single lag, all domains (for supplementary)
#   Rscript RidgeLine_NDD.R --mode domains_single --lag 10 \
#     --output Result/Ridgeline/Fig_NDD_Lag10_Domains.png
#
# ============================================================================
...
# NDD-specific file pattern (G20_G30_G12.2_F01_F03)
NDD_PATTERN <- "G20_G30_G12.2_F01_F03"
NDD_LABEL <- "Neurodegenerative Diseases"
...
DOMAIN_PALETTES <- list(
  ...
  "Air" = c("#E8F5E9", "#A5D6A7", "#66BB6A", "#43A047", "#1B5E20"),
  "Water" = c("#E3F2FD", "#90CAF9", "#42A5F5", "#1E88E5", "#0D47A1"), # Blue
  "Land" = c("#EFEBE9", "#BCAAA4", "#8D6E63", "#6D4C41", "#4E342E"), # Brown
  "Built" = c("#F5F5F5", "#D9D9D9", "#BDBDBD", "#969696", "#525252"), # Grey
  "Social" = c("#F3E5F5", "#CE93D8", "#AB47BC", "#8E24AA", "#4A148C") # Violet
)
...
PALETTES <- list(
  ...
  # NDD-specific palette (green-teal-blue gradient)
  ndd = c("#376795", "#528fad", "#72bcd5", "#aadce0", "#ffe6b7", "#ffd06f",
...
#' Load and process ridgeline data for NDD
...
# ============================================================================
# Mode 1: All Domains by Lag (Main Figure) - NDD
# ============================================================================

plot_domains_by_lag <- function(project_root, output_file, palette = "warm") {
  cat("\n════════════════════════════════════════════════════════════\n")
  cat("Mode: Domains by Lag (Publication Main Figure) - NDD\n")
  cat("════════════════════════════════════════════════════════════\n\n")

  # Load all MultiDomain files for NDD
  ridge_dir <- file.path(project_root, "Result/Ridgeline")
  files_multi <- list.files(ridge_dir, pattern = paste0(NDD_PATTERN, ".*MultiDomain\\.rds$"), full.names = TRUE)
  files_overall <- list.files(ridge_dir, pattern = paste0(NDD_PATTERN, ".*Overall\\.rds$"), full.names = TRUE)

  if (length(files_multi) == 0) {
    stop("No NDD MultiDomain files found")
  }

  cat("Loading NDD MultiDomain models:", length(files_multi), "files\n")
  cat("Loading NDD Overall models:", length(files_overall), "files\n\n")
...
# ============================================================================
# Mode 2: Single Lag, All Domains (Supplementary Figure) - NDD
# ============================================================================

plot_domains_single_lag <- function(project_root, lag, output_file, palette = "warm") {
  cat("\n════════════════════════════════════════════════════════════\n")
  cat("Mode: Single Lag, All Domains (Supplementary Figure) - NDD\n")
  cat("════════════════════════════════════════════════════════════\n\n")

  ridge_dir <- file.path(project_root, "Result/Ridgeline")

  # Find NDD files for this lag
  file_multi <- list.files(ridge_dir,
    pattern = sprintf("%s.*Lag%d_MultiDomain\\.rds$", NDD_PATTERN, lag),
    full.names = TRUE
  )
  file_overall <- list.files(ridge_dir,
    pattern = sprintf("%s.*Lag%d_Overall\\.rds$", NDD_PATTERN, lag),
    full.names = TRUE
  )

  if (length(file_multi) == 0) {
    stop(sprintf("No NDD MultiDomain file found for Lag %d", lag))
  }

  cat("Loading NDD Lag", lag, "models\n\n")
...
# ============================================================================
# Mode 3: Overall Evolution (Dose-Response Over Time) - NDD
# ============================================================================

plot_overall_evolution <- function(project_root, output_file, palette = "ndd") {
  cat("\n════════════════════════════════════════════════════════════\n")
  cat("Mode: Overall EQI Evolution Across Lags - NDD\n")
  cat("════════════════════════════════════════════════════════════\n\n")
  ...
  if (length(files_overall) == 0) {
    stop("No NDD Overall model files found")
  }
...
# ============================================================================
# Mode 3b: Overall Evolution v2 (with Y-axis labels and MRD color mapping) - NDD
# ============================================================================

plot_overall_evolution_v2 <- function(project_root, output_file) {
  cat("\n════════════════════════════════════════════════════════════\n")
  cat("Mode: Overall EQI Evolution Across Lags (Version 2) - NDD\n")
  cat("════════════════════════════════════════════════════════════\n\n")
  ...
  # Plot with NDD-specific purple color scheme
  cat("Creating temporal evolution plot (v2) - NDD...\n")
  ...
  p <- ggplot(combined, aes(x = effect, y = quintile_label, fill = after_stat(x))) +
    ...
    scale_fill_gradientn(
      colors = c("#376795", "#528fad", "#72bcd5", "#aadce0", "#ffe6b7", "#ffd06f",
        "#f7aa58", "#ef8a47", "#e76254"),
      name = "MRD",
      limits = c(0, 30),
      guide = "none"
...
# ============================================================================
# Mode 4: Lags by Domains (Transposed Figure) - NDD
# ============================================================================

plot_lags_by_domains <- function(project_root, output_file, palette = "green") {
  cat("\n════════════════════════════════════════════════════════════\n")
  cat("Mode: Lags by Domains (Split by Lag) - NDD\n")
  cat("════════════════════════════════════════════════════════════\n\n")

  # Load all NDD MultiDomain files
  ridge_dir <- file.path(project_root, "Result/Ridgeline")
  files_multi <- list.files(ridge_dir, pattern = paste0(NDD_PATTERN, ".*MultiDomain\\.rds$"), full.names = TRUE)
  files_overall <- list.files(ridge_dir, pattern = paste0(NDD_PATTERN, ".*Overall\\.rds$"), full.names = TRUE)
  ...
  for (current_lag in c(5, 10, 15)) {
    ...
    # Save
    current_output <- file.path(project_root, sprintf("Result/Ridgeline/Ridgeline_NDD_lag%d_Domain.png", current_lag))
...
main <- function() {
  ...
  cat("\n")
  cat("╔════════════════════════════════════════════════════════════╗\n")
  cat("║   Publication-Quality Ridgeline Plots - NDD               ║\n")
  cat("║   Neurodegenerative Diseases (G20_G30_G12.2_F01_F03)      ║\n")
  cat("║   Design: Nature / The Lancet Standards                   ║\n")
  cat("║   Running all modes automatically                         ║\n")
  cat("╚════════════════════════════════════════════════════════════╝\n")

  # Run Ridgeline_NDD_lag2.png (overall evolution v2)
  plot_overall_evolution_v2(
    project_root,
    file.path(project_root, "Result/Ridgeline/Ridgeline_NDD_lag2.png")
  )

  # Run Ridgeline_NDD_lag{5/10/15}_Domain.png (lags by domains)
  plot_lags_by_domains(project_root, NULL, "ndd")

  cat("\n")
  cat("════════════════════════════════════════════════════════════\n")
  cat("✅ All NDD publication figures generated successfully\n")
  cat("════════════════════════════════════════════════════════════\n\n")
  cat("Generated files:\n")
  cat("  - Result/Ridgeline/Ridgeline_NDD_lag2.png\n")
  cat("  - Result/Ridgeline/Ridgeline_NDD_lag5_Domain.png\n")
  cat("  - Result/Ridgeline/Ridgeline_NDD_lag10_Domain.png\n")
  cat("  - Result/Ridgeline/Ridgeline_NDD_lag15_Domain.png\n\n")
}
