...
apply_manual_quintile_adjustments <- function(data, outcome_name) {
  # Relabel only the plotting data; the fitted-model RDS files are untouched.
  relabel <- function(x, domain_name, lag_values, map) {
    idx <- x$domain == domain_name & x$lag %in% lag_values
    x$quintile[idx] <- unname(map[x$quintile[idx]])
    x
  }
  swap_q3_q5 <- c(Q2 = "Q2", Q3 = "Q5", Q4 = "Q4", Q5 = "Q3")
  swap_q4_q5 <- c(Q2 = "Q2", Q3 = "Q3", Q4 = "Q5", Q5 = "Q4")
  swap_q2_q3 <- c(Q2 = "Q3", Q3 = "Q2", Q4 = "Q4", Q5 = "Q5")
  cycle_q5_q2_q3_q4 <- c(Q2 = "Q3", Q3 = "Q4", Q4 = "Q5", Q5 = "Q2")
  cycle_q5_q3_q4 <- c(Q2 = "Q2", Q3 = "Q4", Q4 = "Q5", Q5 = "Q3")

  if (outcome_name == "C34") {
    data <- relabel(data, "Air", 10, swap_q3_q5)
    data <- relabel(data, "Air", 15, cycle_q5_q2_q3_q4)
  } else if (outcome_name == "J43_J44") {
    data <- relabel(data, "Air", 5, swap_q4_q5)
    data <- relabel(data, "Air", c(10, 15), cycle_q5_q2_q3_q4)
  } else if (outcome_name == "J84_D86") {
    data <- relabel(data, "Air", 5, swap_q4_q5)
    data <- relabel(data, "Air", 15, cycle_q5_q3_q4)
  } else if (outcome_name == "N18_N19") {
    data <- relabel(data, "Air", 5, swap_q2_q3)
  } else if (outcome_name == "C64_C65") {
    # "C65_C65" in the request corresponds to the available C64_C65 files.
    data <- relabel(data, "Air", 5, swap_q3_q5)
    data <- relabel(data, "Built", 10, swap_q3_q5)
    data <- relabel(data, "Air", 15, cycle_q5_q3_q4)
    data <- relabel(data, "Built", 15, swap_q3_q5)
    data <- relabel(data, "Air", 20, swap_q3_q5)
    data <- relabel(data, "Built", 20, swap_q3_q5)
  }
...
process_outcome <- function(outcome_name, project_root) {
  ...
  output_dir <- file.path(project_root, sprintf("Result/brms_RL_Visualization_CRD_CKD_Single/%s", outcome_name))
  dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

  brms_dir <- file.path(project_root, "Result/brms_RL_CRD_CKD_Single")
...
main <- function() {
  ...
  outcomes <- if (length(requested_outcomes) > 0) {
    ...
    c("C34", "C64_C65", "J43_J44", "J84_D86", "N00_N15", "N18_N19")
  }
  ...
  cat("Output: Result/brms_RL_Visualization_CRD_CKD_Single/{Outcome}/\n")
