"""
EQI LMM Visualization - Land Use Cluster Stratification

- Reads LMM results from Result/brms_Typology_LandUse (paths via config.yaml)
- Builds forest-plot style figures for Land Use Cluster stratifications
- Output: {ICD}_{LandUse}_{EQI_Period}_{AAMR_Period}_Lag{lag}.png
...
def get_paths(project_root: Path, cfg: dict) -> Dict[str, Path]:
    """Get paths for LandUse results and visualization output"""
    result_dir = project_root / "Result" / "brms_Typology_LandUse"
    vis_dir = project_root / "Result" / "brms_LandUse_Visualization"
...
def list_icds_from_results(result_dir: Path) -> List[str]:
    """List ICD codes from LandUse result files"""
    icds = []
    for p in sorted(result_dir.glob("*_LandUse.csv")):
        stem = p.stem.replace("_LandUse", "")
        if stem not in icds:
            icds.append(stem)
    return icds
...
def plot_landuse_forest(
    ...
    """Create forest plot for Land Use Cluster stratification"""

    # Define panel configurations (for matching data in CSV)
    panel_labels = [
        "Natural",
        "Water_Sensitive",
        "Agricultural",
    ...
    # Define display labels (for plot titles)
    ...
    # Create figure - 4 panels for land use types
    ...
    # Format title
    title = f"({scenario_label}) {cancer_name} {lag}-year Lag - Land Use Clusters"
    fig.suptitle(title, fontsize=14, y=0.94)

    # For each panel (land use stratum)
    for panel_idx, panel_label in enumerate(panel_labels):
        ax = axes[panel_idx]

        # Filter data for current panel
        landuse_models = [
            f"LandUse_{panel_label}_EQI",
            f"LandUse_{panel_label}_EQI_Air",
            f"LandUse_{panel_label}_EQI_Water",
            f"LandUse_{panel_label}_EQI_Land",
            f"LandUse_{panel_label}_EQI_Built",
            f"LandUse_{panel_label}_EQI_Social",
        ]

        panel_df = df[df["Model"].isin(landuse_models)].copy()

        # Collect all values for y-axis scaling
        panel_values = []

        # Parse data and collect values
        for model_idx, model in enumerate(landuse_models):
        ...
        # Set y-axis label
        if panel_idx == 2:  # Middle panel (between 0-3)
            ax.set_ylabel(
        ...
        # Add panel label in top right corner (use display label)
        ax.text(
            0.98,
            0.95,
            panel_display_labels[panel_idx],
            fontsize=11,
    ...
    if output_dir:
        ...
        output_filename = (
            output_path / f"{icd_code}_LandUse_{eqi_period}_{aamr_period}_Lag{lag}.png"
        )
    else:
        output_filename = f"{icd_code}_LandUse_{eqi_period}_{aamr_period}_Lag{lag}.png"

    plt.savefig(output_filename, dpi=300, bbox_inches="tight")
    print(f"LandUse forest plot saved as {output_filename}")
    plt.close()
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate Land Use Cluster stratified forest plots"
    )
    parser.add_argument(
    ...
    for icd in icds:
        result_csv = paths["result"] / f"{icd}_LandUse.csv"
        ...
        for eqi, aamr, lag in SCENARIO_ORDER:
            ...
            try:
                ...
                print(f"Generated plot: {icd}_LandUse_{eqi}_{aamr}_Lag{lag}.png")
