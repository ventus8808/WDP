...
class LMMResultFormatter:
    ...
    def format_coefficient(self, coef_info: Dict) -> str:
        """
        格式化单个系数及其置信区间
    ...
    def create_result_table(self, model_results: Dict, cancer_types: List[str], scenarios: List[str]) -> pd.DataFrame:
        """
        创建结果表格
                ...
                for model_name in self.model_order:
                    if model_name in scenario_results:
                        ...
                        row = {
                            ...
                            'Q1': self.format_coefficient(coeffs.get('Q1', {})),
                            'Q2': self.format_coefficient(coeffs.get('Q2', {})),
                            'Q3': self.format_coefficient(coeffs.get('Q3', {})),
                            'Q4': self.format_coefficient(coeffs.get('Q4', {})),
                            'Q5': self.format_coefficient(coeffs.get('Q5', {}))
                        }
    ...
    def save_results(self, model_results: Dict, cancer_types: List[str], output_dir: str) -> Dict[str, str]:
        """
        保存结果表格到文件 - 按癌症类型分别输出
        ...
        # 为每个癌症类型创建单独的结果表
        for cancer_type in cancer_types:
            cancer_table = self.create_result_table(model_results, [cancer_type], all_scenarios)
