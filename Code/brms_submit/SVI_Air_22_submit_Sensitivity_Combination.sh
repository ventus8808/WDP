#!/bin/bash
# Slurm array launcher for the EQI-Air × SVI 2x2 joint sensitivity combination.
# One task per overall outcome (from outcome_overall.list); each task runs all
# 64 covariate combinations × 1 scenario (AAMR 2011-2015, lag 10) internally.
# Usage:
#   bash Code/brms_submit/SVI_Air_22_submit_Sensitivity_Combination.sh

#SBATCH --partition=wzhctest
#SBATCH --job-name=brms_SVI_Air_22_Sensitivity_Combination
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=6
#SBATCH --mem=18G
#SBATCH --time=2-00:00:00
#SBATCH --output=SVI_Air_22_Sensitivity_Combination_%A_%a.out
#SBATCH --error=SVI_Air_22_Sensitivity_Combination_%A_%a.err
...
RUNNER="Code/brms/SVI_Air_22_brms_Sensitivity_Combination.R"
