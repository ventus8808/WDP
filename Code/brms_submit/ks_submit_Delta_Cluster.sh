#!/bin/bash
# Slurm array launcher for the cluster-stratified delta mixed model pipeline.
# One task per outcome; each task runs all lag × Cluster_EQI/NLCD × domain models.
# Usage:
#   bash Code/brms_submit/ks_submit_Delta_Cluster.sh
...
#SBATCH --output=Delta_Cluster_%A_%a.out
#SBATCH --error=Delta_Cluster_%A_%a.err
...
RUNNER="Code/brms/Delta_Cluster.R"
