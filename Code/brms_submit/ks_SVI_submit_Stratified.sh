#!/bin/bash
# Slurm array launcher for the comprehensive SVI stratified pipeline.
# One task per outcome (from outcome.list); each task runs all four
# stratification phases (Cluster -> Demo -> Geo -> RUCC) internally,
# across the 4 SVI lag scenarios.
# Usage:
#   bash Code/brms_submit/ks_SVI_submit_Stratified.sh

#SBATCH --partition=kshctest02
#SBATCH --job-name=brms_SVI_Stratified
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=6
#SBATCH --mem=18G
#SBATCH --time=2-00:00:00
#SBATCH --output=SVI_Stratified_%A_%a.out
#SBATCH --error=SVI_Stratified_%A_%a.err
...
RUNNER="Code/brms/SVI_brms_Stratified.R"
if [ ! -f "$RUNNER" ]; then
