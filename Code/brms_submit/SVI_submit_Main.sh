#!/bin/bash
# Slurm array launcher for the SVI interval-censored mixed model pipeline
# One task per outcome; each task runs all four lags inside the R runner.
# Usage:
#   bash Code/brms_submit/SVI_submit_Main.sh

#SBATCH --partition=wzhctest
#SBATCH --job-name=brms_SVI_Main
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=6
#SBATCH --mem=18G
#SBATCH --time=1-00:00:00
#SBATCH --output=SVI_Main_%A_%a.out
#SBATCH --error=SVI_Main_%A_%a.err
...
RUNNER="Code/brms/SVI_brms_Main.R"
...
# Run the SVI interval-censored pipeline for this outcome
Rscript "$RUNNER" \
  --outcomes "$OUTCOME"
