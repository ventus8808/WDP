#!/bin/bash
# Slurm array launcher for the cmdstan interaction cluster interval-censored mixed model pipeline
# One task per cancer type; each task runs the interaction model for clusters.
# Usage:
#   bash Code/brms/submit_cmdstan_interaction_cluster_array.sh         # auto-discovers combinations and submits an array
#   # or, advanced: sbatch --array=0-<N-1> Code/brms/submit_cmdstan_interaction_cluster_array.sh

#SBATCH --partition=kshctest
#SBATCH --job-name=WDP_cmdstan_interaction_cluster
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --mem=48G
#SBATCH --time=1-00:00:00
#SBATCH --output=cmdstan_interaction_cluster_%A_%a.out
#SBATCH --error=cmdstan_interaction_cluster_%A_%a.err
...
RUNNER="Code/brms/cmdstan_Interaction_Cluster.R"
...
# Run the interaction cluster model
log INFO "运行交互聚类模型"
Rscript "$RUNNER" \
