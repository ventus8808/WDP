#!/bin/bash
# Slurm array launcher for the SVI ridgeline posterior extraction pipeline.
# One task per outcome (from outcome_overall.list); each task runs all 4 SVI lag
# scenarios and saves {shortname}_Lag{N}_SVI.rds files.
# Usage:
#   bash Code/brms_submit/SVI_submit_Main_RL.sh

#SBATCH --partition=wzhctest
#SBATCH --job-name=brms_SVI_Main_RL
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=6
#SBATCH --mem=18G
#SBATCH --time=1-00:00:00
#SBATCH --output=SVI_Main_RL_%A_%a.out
#SBATCH --error=SVI_Main_RL_%A_%a.err
...
RUNNER="Code/brms/SVI_brms_Main_RL.R"
