#!/bin/bash
# Slurm array launcher for the delta main interval-censored mixed model pipeline.
# One task per outcome; each task runs all lag × domain models inside the R runner.
# Usage:
#   bash Code/brms_submit/ks_submit_Delta_Main.sh

#SBATCH --partition=kshctest02
#SBATCH --job-name=WDP_Delta_Main
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=12G
#SBATCH --time=0-12:00:00
#SBATCH --output=Delta_Main_%A_%a.out
#SBATCH --error=Delta_Main_%A_%a.err
