#!/bin/bash
# Slurm array launcher for the cluster-stratified interval-censored mixed model pipeline.
# One task per outcome; each task runs all strat-vars × strat-vals × scenarios inside the R runner.
# Usage:
#   bash Code/brms_submit/ks_submit_Stratified_Cluster.sh

#SBATCH --partition=kshctest02
#SBATCH --job-name=WDP_Stratified_Cluster
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=6
#SBATCH --mem=18G
#SBATCH --time=1-00:00:00
#SBATCH --output=Stratified_Cluster_%A_%a.out
#SBATCH --error=Stratified_Cluster_%A_%a.err
...
RUNNER="Code/brms/brms_Stratified_Cluster.R"
