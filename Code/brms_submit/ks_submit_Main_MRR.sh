#!/bin/bash
# Slurm array launcher for the MRR pipeline.
# One task per outcome (from outcome.list); each task runs all
# EQI 2000-2005 lag scenarios internally.
# Usage:
#   bash Code/brms_submit/ks_submit_Main_MRR.sh

#SBATCH --partition=kshctest02
#SBATCH --job-name=brms_Main_MRR
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=6
#SBATCH --mem=18G
#SBATCH --time=1-00:00:00
#SBATCH --output=Main_MRR_%A_%a.out
#SBATCH --error=Main_MRR_%A_%a.err
...
RUNNER="Code/brms/brms_Main_MRR.R"
if [ ! -f "$RUNNER" ]; then
