...
DF_OUTPUT = (
    PROJECT_ROOT
    / CFG.get("eqi_aamr_outputs", {}).get("base_dir", "Data/Processed/df_EQI_AAMR")
    / CFG.get("eqi_aamr_outputs", {}).get("eqi_aamr_interval", "EQI_AAMR_Interval.csv")
)
EQI_DIR = PROJECT_ROOT / CFG["data_sources"]["epa_eqi"]["processed"]
SUPPRESSION_THRESHOLD = float(
    CFG["data_sources"]["cdc_wonder"].get("eqi_suppression_threshold", 40.0)
)
SMOKING_PATH = (
    PROJECT_ROOT
...
def _clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_clean = df_clean[
        (df_clean["Deaths"] != "Missing") & (df_clean["Population"] != "Missing")
    ]
    return df_clean
...
def _get_ci_cols(df: pd.DataFrame):
    lcl_col = _find_col_by_keywords(
        df, ["age", "adjusted", "rate", "lower", "confidence"]
    )
    if lcl_col is None:
        for alias in [
            ...
            if alias in df.columns:
                lcl_col = alias
                break
    ucl_col = _find_col_by_keywords(
        df, ["age", "adjusted", "rate", "upper", "confidence"]
    )
    if ucl_col is None:
        for alias in [
    ...
    rate_col = _find_col_by_keywords(
        df,
        ["age", "adjusted", "rate"],
        ["lower", "upper", "confidence", "interval", "ci", "standard", "error", "se"],
    )
    if rate_col is None:
        for alias in [
            "Age Adjusted Rate",
    ...
    for code in ("0005", "0610"):
        ...
        if fp.exists():
            ...
            if columns_to_rename:
                ...
                print(
                    f"  📝 EQI{code}: 映射列名 {list(columns_to_rename.keys())} -> {list(columns_to_rename.values())}"
                )
...
def _map_eqi_period(time_period: str, lag_years: int):
    ...
    if time_period == "2011-2015":
        return (
            "2000-2005" if lag_years == 10 else "2006-2010" if lag_years == 5 else None
        )
...
def _compute_interval_rows(dfc: pd.DataFrame) -> pd.DataFrame:
    ...
    suppressed_mask = deaths_txt.str.contains(
        "suppress", na=False
    ) | deaths_num.between(1, 9, inclusive="both")

    rate_col, lcl_col, ucl_col = _get_ci_cols(dfc)
    point = (
        pd.to_numeric(dfc[rate_col], errors="coerce")
        if rate_col
        else pd.Series([pd.NA] * len(dfc), dtype="Float64", index=dfc.index)
    )
    lcl = (
        pd.to_numeric(dfc[lcl_col], errors="coerce")
        if lcl_col
        else pd.Series([pd.NA] * len(dfc), dtype="Float64", index=dfc.index)
    )
    ucl = (
        pd.to_numeric(dfc[ucl_col], errors="coerce")
        if ucl_col
        else pd.Series([pd.NA] * len(dfc), dtype="Float64", index=dfc.index)
    )
...
def main():
    ...
    qualified_icds = [
        icd
        for icd, d in icd_rates.items()
        if len(d) == len(grouped)
        and all(v <= SUPPRESSION_THRESHOLD for v in d.values())
    ]
    ...
    for period in sorted(grouped.keys()):
        pairs = [
            (icd, fp)
            for icd, fp in grouped[period]
            if (not qualified_icds) or (icd in qualified_icds)
        ]
        if not pairs:
            continue
        ...
        for icd, fp in pairs:
            try:
                ...
                dfc["COUNTY_FIPS"] = (
                    dfc["County Code"].astype(int).astype(str).str.zfill(5)
                )
                dfc["State"] = (
                    dfc.get("County", "").apply(_extract_state_from_county)
                    if "County" in dfc.columns
                    else ""
                )
                ...
                # 3. 2000-2005 EQI (code '0005') + 15 years lag -> 2016-2020 AAMR
                # 4. 2006-2010 EQI (code '0610') + 5 years lag -> 2011-2015 AAMR
                # 5. 2006-2010 EQI (code '0610') + 10 years lag -> 2016-2020 AAMR
                valid_combinations = []
                if period == "2006-2010":
                    ...
                    # Both 2000-2005 EQI with 15 years lag and 2006-2010 EQI with 10 years lag
                    valid_combinations.append((15, "0005", "2000-2005"))
