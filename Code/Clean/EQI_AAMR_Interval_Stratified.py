...
Also outputs stratified data by Sex and Race:
- EQI_AAMR_C00_C97_Sex.csv - Sex-stratified data
- EQI_AAMR_C00_C97_Race.csv - Race-stratified data
...
# 新增分层数据输出路径
DF_OUTPUT_SEX = PROJECT_ROOT / CFG.get('eqi_aamr_outputs', {}).get('base_dir', 'Data/Processed/df_EQI_AAMR') / 'EQI_AAMR_C00_C97_Sex.csv'
DF_OUTPUT_RACE = PROJECT_ROOT / CFG.get('eqi_aamr_outputs', {}).get('base_dir', 'Data/Processed/df_EQI_AAMR') / 'EQI_AAMR_C00_C97_Race.csv'
# 分层数据源目录
STRATIFIED_SRC_DIR = PROJECT_ROOT / "Data/Original/CDC WONDER EQI Stratified"
...
def _compute_interval_rows(dfc: pd.DataFrame) -> tuple:
...
def _process_stratified_data(file_pattern, stratification_col, stratification_code_col, output_file):
    """处理分层数据"""
    print(f'🧮 Building {stratification_col} stratified AAMR interval table...')
    files = list(STRATIFIED_SRC_DIR.glob(file_pattern))
    if not files:
        print(f'⚠️ No files matching {file_pattern} under {STRATIFIED_SRC_DIR}')
        return
    ...
    for fp in files:
        try:
            # 提取时间周期
            period_match = re.search(r'(\d{4}-\d{4})', fp.name)
            if not period_match:
                continue
            ...
            dfc['State'] = dfc['County'].apply(_extract_state_from_county) if 'County' in dfc.columns else pd.Series([''] * len(dfc))
            
            # 获取分层变量
            if stratification_col in dfc.columns:
                stratification_values = dfc[stratification_col].unique()
                for strat_val in stratification_values:
                    if pd.isna(strat_val):
                        continue
                    strat_df = dfc[dfc[stratification_col] == strat_val].copy()
                    
                    lo, hi = _compute_interval_rows(strat_df)
                    base = pd.DataFrame({
                        'COUNTY_FIPS': strat_df['COUNTY_FIPS'].values,
                        'State': strat_df['State'].values,
                        'Time_Period': [period] * len(strat_df),
                        'Cancer_Type': ['C00_C97'] * len(strat_df),  # 固定为C00-C97
                        stratification_col: [strat_val] * len(strat_df),
                        'AAMR_lower': lo.values,
                        'AAMR_upper': hi.values,
                    })
                    base = base.drop_duplicates(subset=['COUNTY_FIPS'], keep='first')
                    
                    # 添加分层编码列（如果存在）
                    if stratification_code_col and stratification_code_col in strat_df.columns:
                        base[stratification_code_col] = strat_df[stratification_code_col].values
                ...
                print(f'  ⚠️ Column {stratification_col} not found in {fp.name}')
        except Exception as e:
            print(f'  ⚠️ {fp.name} failed: {e}')
            continue
    
    if not rows:
        print(f'⚠️ No {stratification_col} stratified rows to write.')
        return
    
    final = pd.concat(rows, ignore_index=True)
    first_cols = ['COUNTY_FIPS','State','EQI_Period','Time_Period','Lag_Years','Cancer_Type',stratification_col]
    if stratification_code_col and stratification_code_col in final.columns:
        first_cols.append(stratification_code_col)
    first_cols.extend(['AAMR_lower','AAMR_upper','Smoking_Rate'])
    ...
    print(f'💾 Saved {len(final):,} {stratification_col} stratified rows to {output_file}')
...
def main():
    ...
    # 处理分层数据
    print('\n🧮 Processing stratified data...')
    # 处理性别分层数据
    _process_stratified_data('*Sex.csv', 'Sex', 'Sex Code', DF_OUTPUT_SEX)
    # 处理种族分层数据
    _process_stratified_data('*Race.csv', 'Race', 'Race Code', DF_OUTPUT_RACE)
