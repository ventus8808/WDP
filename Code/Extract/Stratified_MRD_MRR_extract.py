...
TYPOLOGY_STRATA = [
    ...
    {"label": "County Economic Typology: Manufacturing", "code": "Typology_Manufacturing"},
    ...
    {"label": "County Economic Typology: Nonspecialized","code": "Typology_Nonspecialized"},
]
...
def load_mrd_sex_race(strat_prefix, suffix):
    ...
    df = df.drop_duplicates(subset=["ICD_Code", "EQI_Period", "AAMR_Period", "Lag", "Model"], keep="first")
    ...
    for domain in DOMAIN_ORDER:
        ...
        for _, r in sub.iterrows():
            rows.append([
                domain, r["Lag"],
                format_mrd_cell(r["Q1"]), format_mrd_cell(r["Q2"]),
                format_mrd_cell(r["Q3"]), format_mrd_cell(r["Q4"]),
...
def load_mrd_generic(fpath, icd_code, strata_list):
    ...
    df = df.drop_duplicates(subset=["ICD_Code", "EQI_Period", "AAMR_Period", "Lag", "Model"], keep="last")
    df["Lag"] = df["Lag"].astype(int)
    df = df[df["Lag"].isin(LAGS)]
    ...
    for s in strata_list:
        ...
        rows  = []
        for domain in DOMAIN_ORDER:
            model_col = f"{code}_{domain}" if domain == "EQI" else f"{code}_EQI_{domain}"
            sub = df[df["Model"] == model_col].sort_values("Lag")
            for _, r in sub.iterrows():
                rows.append([
                    domain, r["Lag"],
...
def extract_mrr_rows(pivot, icd_filter, strata_code):
    ...
    rows = []
    for domain in DOMAIN_ORDER:
        model_col = f"{strata_code}_{domain}" if domain == "EQI" else f"{strata_code}_EQI_{domain}"
...
def load_mrr_generic(full_icd, file_suffix, strata_list):
    ...
    with open(out_path, "w") as fh:
        ...
        for s in SEX_RACE_STRATA:
            ...
            if kind == "mrd":
                ...
                rows = load_mrr_sex_race(full_icd, strat_pref, suffix)
        ...
        if kind == "mrd":
            ...
            rucc_data = load_mrr_generic(full_icd, "RUCC", RUCC_STRATA)
        ...
        if kind == "mrd":
            ...
            koppen_data = load_mrr_generic(full_icd, "Koppen", KOPPEN_STRATA)
        ...
        if kind == "mrd":
            ...
            census_data = load_mrr_generic(full_icd, "CensusRegion", CENSUS_STRATA)
        ...
        if kind == "mrd":
            ...
            typo_data = load_mrr_generic(full_icd, "Typology", TYPOLOGY_STRATA)
