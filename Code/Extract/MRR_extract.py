...
Outputs:
  Result/Tables/MRR.csv   — MRR only (backward compat for Visualization_MRR.py)
  Result/Tables/main.csv  — MRR + MRD merged (long format)
...
# ---------------------------------------------------------------------------
# Extract MRR  (*_MRR.csv → long format)
# ---------------------------------------------------------------------------
mrr_frames = []
for fname in sorted(f for f in os.listdir(input_dir) if f.endswith("_MRR.csv")):
    ...
    if icd_code in SKIP_CODES:
        continue
    mrr_frames.append(pd.read_csv(os.path.join(input_dir, fname)))
...
mrr.to_csv(os.path.join(output_dir, "MRR.csv"), index=False)
print(f"Saved MRR.csv  ({len(mrr)} rows)")

# ---------------------------------------------------------------------------
# Extract MRD  (*_main.csv → parse wide Q2–Q5 → long format)
# ---------------------------------------------------------------------------
mrd_frames = []
for fname in sorted(f for f in os.listdir(input_dir) if f.endswith("_main.csv")):
    ...
    # Strip stray quotes from Model column
    ...
    if rows:
        mrd_frames.append(pd.DataFrame(rows))

if mrd_frames:
    mrd = pd.concat(mrd_frames, ignore_index=True)

    # Merge MRR + MRD
    mrr_merge = mrr.rename(columns={"p": "p_MRR"})
    merged = mrr_merge.merge(
        mrd[["ICD_Code", "Lag", "Quintile", "MRD_mean", "MRD_lower", "MRD_upper", "p_MRD"]],
        on=["ICD_Code", "Lag", "Quintile"],
        how="left",
    )
    merged = sort_df(merged)
    merged.to_csv(os.path.join(output_dir, "main.csv"), index=False)
    print(f"Saved main.csv ({len(merged)} rows)")
    print(
    ...
    print("No *_main.csv files found — main.csv not written.")
