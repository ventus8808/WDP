...
utils::globalVariables(c("EQI", "State_FIPS",
...
option_list <- list(
  ...
  make_option(c("--output-dir"),    type = "character", default = "Result/brms_MRR",        help = "Output directory"),
...
extract_covariate <- function(draw_df, names_vec, col_name, summ_df) {
  ...
  col <- paste0("beta[", idx, "]")
  sr  <- summ_df[summ_df$variable == col, , drop = FALSE]
  list(est      = format_cell(draw_df[[col]]),
...
for (outcome in selected) {
  ...
  for (sc in scenario_list) {
    ...
    if (nrow(des$df) < opt$`min-n`) {
      message("[Skip] ", scen_key, " after design n=", nrow(des$df)); next
    }
    states <- sort(unique(des$df$State_FIPS))
    si     <- match(des$df$State_FIPS, states)
    dl <- list(N = nrow(des$df), S = length(states), state = si,
    ...
    if (inherits(fit, "try-error")) { message("[Fail] ", scen_key); next }
