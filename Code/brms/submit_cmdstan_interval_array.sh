#!/bin/bash
# Slurm array launcher for the cmdstan interval-censored mixed model pipeline
# One task per Cancer_Type; each task runs all scenarios and layers inside the R runner.
# Usage:
#   bash Code/brms/submit_cmdstan_interval_array.sh         # auto-discovers diseases and submits an array
#   # or, advanced: sbatch --array=0-<N-1> Code/brms/submit_cmdstan_interval_array.sh

#SBATCH --partition=kshctest
#SBATCH --job-name=WDP_cmdstan_interval
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
DATA_CSV="Data/Processed/df_EQI_AAMR/EQI_AAMR_Interval.csv"
...
# Run the interval-censored pipeline for this disease
Rscript "$RUNNER" \
  --cancer-types "$CANCER" \
