#!/bin/bash
# Slurm array launcher for cmdstan_main_MRR_log.R (log-linear model)
# One task per cancer type; each task runs 3 EQI0005 scenarios (lags 5/10/15),
# Overall EQI model only, outputs within-lag MRR + lag test to Result/brms_MRR_lag_log/.
# Usage:
#   bash Code/brms/submit_cmdstan_main_MRR_log.sh    # submit array to Slurm
#   # or, advanced: sbatch --array=0-<N-1> Code/brms/submit_cmdstan_main_MRR_log.sh

#SBATCH --partition=kshctest
#SBATCH --job-name=WDP_MRR_log
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --mem=48G
#SBATCH --time=1-00:00:00
#SBATCH --output=cmdstan_MRR_log_%A_%a.out
#SBATCH --error=cmdstan_MRR_log_%A_%a.err
...
RUNNER="Code/brms/cmdstan_main_MRR_log.R"
if [ ! -f "$RUNNER" ]; then
...
Rscript "$RUNNER" \
  ...
  --output-dir "Result/brms_MRR_lag_log" \
  --chains 4 --iter 2000 --warmup 1000 \
  --adapt-delta 0.95 --max-treedepth 12 \
  --seed "$SEED"

log INFO "✅ 完成: Cancer=$CANCER_TYPE"
log INFO "输出目录: Result/brms_MRR_lag_log/"
