#!/usr/bin/env Rscript
# Interactive effect analysis using cmdstanr interval-censored mixed model
# Based on cmdstan_interval_runner.R, extended for Quintile * Cluster interactions
# Output: Result/brms_Interaction/{ICD-Code}_Interaction.csv per cancer type
...
option_list <- list(
  make_option(c("--data"), type="character", default="Data/Processed/df_EQI_AAMR/EQI_AAMR_Interval_Clustered.csv", help="Input clustered interval data"),
  make_option(c("--output-dir"), type="character", default="Result/brms_Interaction", help="Output directory"),
...
for(cancer in selected){
  ...
  outfile <- file.path(out_dir, paste0(cancer, "_Interaction.csv"))
  ...
  for(sc_idx in seq_along(scenario_list)){
    ...
    for(dom_idx in seq_along(domains)){
      ...
      # Output rows for each cluster
      clusters <- c("Advantageous","Unbalanced","Disadvantaged")
      for(cl_idx in seq_along(clusters)){
        cl <- clusters[cl_idx]
        int_p_val <- if(cl == "Advantageous") "(Baseline)" else res$int_p[[cl]]
        row <- tibble(
          ICD_Code = cancer,
          EQI_Period = eqi_out,
