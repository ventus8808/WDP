#!/usr/bin/env Rscript
# Pure cmdstanr interval-censored mixed model pipeline stratified by climate/geographic factors
# Output format mimics LMM results (ICD_Code, EQI_Period, AAMR_Period, Lag, Model, Q1..Q5)
# Uses stratification variables (e.g., census_region) instead of cluster IDs
...
option_list <- list(
  make_option(c("--data"), type="character", default="Data/Processed/df_EQI_AAMR/EQI_AAMR_Interval_Climate.csv", help="Input interval data with stratification variables"),
  make_option(c("--output-dir"), type="character", default="Result/brms_Climate", help="Output directory"),
  make_option(c("--cancer-types"), type="character", default="C00_C97", help="Comma separated ICD codes (fixed to C00_C97)"),
  make_option(c("--stratification-var"), type="character", default=NULL, help="Stratification variable (e.g., census_region)"),
  make_option(c("--stratification-values"), type="character", default=NULL, help="Comma separated values for stratification (optional, defaults to all unique values)"),
...
# Parse stratification variable
if (is.null(opt$`stratification-var`)) stop("Stratification variable must be specified")
strat_var <- opt$`stratification-var`
if (!strat_var %in% names(dt)) stop("Stratification variable not found in data: ", strat_var)

# Parse stratification values
strat_values <- if (!is.null(opt$`stratification-values`)) {
  str_split(opt$`stratification-values`,",",simplify=TRUE) |> as.vector() |> str_trim() |> as.integer()
} else {
  unique(dt[[strat_var]]) |> sort()
}
message("Stratification variable: ", strat_var, " | Values to analyze: ", paste(strat_values,collapse=","))

req <- c("COUNTY_FIPS","EQI_Period","Time_Period","Lag_Years","Cancer_Type","AAMR_lower","AAMR_upper","Smoking_Rate","RUCC","EQI","EQI_Air","EQI_Water","EQI_Land","EQI_Built","EQI_Social", strat_var)
...
# Filter to valid stratification values
dt <- dt[get(strat_var) %in% strat_values]
...
for(cancer in selected){
  ...
  for(strat_value in strat_values){
    strat_title <- paste0(strat_var, "_", strat_value)
    outfile <- file.path(out_dir, paste0(cancer, "_", strat_var, ".csv"))
    strat_dt <- dt[get(strat_var) == strat_value]
    message("Analyzing ", strat_title, " (n=", nrow(strat_dt), ")")
    
    for(sc in scenario_list){
      scen_key <- sc$key; eqi_p <- sc$eqi; aamr_p <- sc$aamr; lagv <- sc$lag
      scen_dt <- strat_dt[EQI_Period==eqi_p & Time_Period==aamr_p & Cancer_Type==cancer]
      ...
      if(inherits(fit_overall,"try-error")){ message("[Fail] Overall EQI model ", scen_key); } else {
        ...
        row_over <- tibble(ICD_Code=cancer, EQI_Period=eqi_out, AAMR_Period=aamr_out, Lag=lagv, Model=paste0(strat_title,"_EQI"), Q1=q_over$Q1,Q2=q_over$Q2,Q3=q_over$Q3,Q4=q_over$Q4,Q5=q_over$Q5)
      ...
      if(inherits(fit_multi,"try-error")){ message("[Fail] Multi-domain model ", scen_key) } else {
        ...
        for(dom in domain_prefix){ qd <- extract_quintiles(draws_m, des_multi$names, dom); row_dom <- tibble(ICD_Code=cancer, EQI_Period=eqi_out, AAMR_Period=aamr_out, Lag=lagv, Model=paste0(strat_title, "_", sub("_factor","",dom)), Q1=qd$Q1,Q2=qd$Q2,Q3=qd$Q3,Q4=qd$Q4,Q5=qd$Q5); append_rows(outfile,row_dom) }
