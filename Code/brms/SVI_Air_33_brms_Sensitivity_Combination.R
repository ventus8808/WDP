#!/usr/bin/env Rscript
# EQI-Air × SVI 3x3 joint sensitivity combination analysis.
# Exposure: EQI_Air_SVI_3 (9 joint levels a1s1..a3s3), a1s1 = reference;
#           output the 8 other levels' mortality rate differences (MRD) vs a1s1.
# All 2^6 = 64 subsets of 6 control covariates, single representative scenario
# (AAMR 2011-2015, lag 10). Same df as the 2x2 script (Data/Processed/df_SVI.csv).
...
utils::globalVariables(c(
  "EQI_Air_SVI_3", "Smoking_rate", "Physical_Activities_rate", "Obesity_rate",
  "Uninsured_rate", "Physician_Density_per100k", "Diabetes_Prevalence_rate", "State_FIPS"
))

# ── Joint exposure configuration (3x3) ──────────────────────────────────────────
JOINT_COL    <- "EQI_Air_SVI_3"
JOINT_LEVELS <- c("a1s1", "a1s2", "a1s3",
                  "a2s1", "a2s2", "a2s3",
                  "a3s1", "a3s2", "a3s3")          # a1s1 = reference (first)
REF_LEVEL    <- "a1s1"
NON_REF      <- setdiff(JOINT_LEVELS, REF_LEVEL)

option_list <- list(
  make_option(c("--data"), type = "character", default = "Data/Processed/df_SVI.csv"),
  make_option(c("--output-dir"), type = "character", default = "Result/brms_SVI_Air_33_Sensitivity_Combination"),
...
for (outcome in selected) {
  ...
  outfile <- file.path(out_dir, paste0(dlabel, "_SVI_Air_33_Sensitivity_Combination.csv"))
