#!/usr/bin/env Rscript
# SVI sensitivity combination analysis: all 2^6 subsets of 6 control covariates
# added to the SVI trajectory exposure.
# Models: +0 covariate (SVI only) = 1; +1 = C(6,1)=6; +2 = 15; +3 = 20;
#         +4 = 15; +5 = 6; +6 = 1 -> total 64 models per scenario.
# Exposure: SVI trajectory category (A/B/C/D), A = reference; output B/C/D MRD.
# SVI is a single static 2000-2022 trajectory class per county, so there is no
# EQI-style period; one representative scenario (AAMR 2011-2015, lag 10) is used.
# Output format: ICD_Code, AAMR_Period, Lag, Model, A/B/C/D + diagnostics.
...
option_list <- list(
  ...
  make_option(c("--output-dir"), type = "character", default = "Result/brms_SVI_Sensitivity_Combination"),
...
# Single representative scenario: SVI baseline trajectory -> AAMR 2011-2015 (lag 10)
scenario_list <- list(
...
combo_list <- list(character(0)) # k=0: SVI only
...
# Per-category (B/C/D vs A) MRD diagnostics from treatment contrasts.
...
# Build design matrix: intercept + optional covariates + SVI_factor (treatment contrasts, A as ref).
...
for (outcome in selected) {
  ...
  outfile <- file.path(out_dir, paste0(dlabel, "_SVI_Sensitivity_Combination.csv"))
  for (sc in scenario_list) {
    ...
    for (combo in combo_list) {
      model_label <- if (length(combo) == 0) "SVI" else paste(c("SVI", covar_abbrev[combo]), collapse = "+")
      ...
      sv <- extract_svi(draws, des$names)
      summ <- posterior::summarize_draws(fit$draws("beta"))
      met <- extract_svi_metrics(draws, des$names, summ)
      ...
      row <- tibble(
        ICD_Code = outcome, AAMR_Period = aamr_out, Lag = lagv, Model = model_label,
