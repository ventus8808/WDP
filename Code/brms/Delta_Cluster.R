...
# stratified by Cluster_EQI (A/B/C) and Cluster_NLCD (A/B/C/D).
# Phase 1: National level (Overall EQI + 5 domains).
# Phase 2: Stratified by Cluster_EQI.
# Phase 3: Stratified by Cluster_NLCD.
...
utils::globalVariables(c(
  "State_FIPS", "Cluster_EQI", "Cluster_NLCD",
...
stan_file <- file.path(tempdir(), "delta_cluster.stan")
...
run_models <- function(d, lag_v, outcome, dlabel, outfile, model_prefix = "") {
  ...
  for (dom in domains) {
    ...
    if (!is.null(rds)) {
      rl_suffix <- if (nchar(model_prefix) > 0) paste0(model_prefix, "_", dom$name) else paste0("Cluster_", dom$name)
...
strat_vars <- c("Cluster_EQI", "Cluster_NLCD")

# ── Main loop ─────────────────────────────────────────────────────────────────
for (outcome in selected) {
  dlabel <- disease_label(outcome)
  outfile <- file.path(out_dir, paste0(dlabel, "_Cluster_EQI.csv"))
  ...
  for (lag_v in lag_vals) {
    ...
    # Phase 2 & 3: Cluster stratification
    for (sv in strat_vars) {
      strat_vals <- sort(na.omit(unique(lag_dt[[sv]])))
      message("  [Phase] Stratify by ", sv, " (", length(strat_vals), " levels)")
      for (val in strat_vals) {
        sub_dt <- lag_dt[!is.na(get(sv)) & get(sv) == val]
        prefix <- paste0(sv, "_", val)
