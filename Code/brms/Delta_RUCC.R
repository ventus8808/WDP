#!/usr/bin/env Rscript
# Interval-censored mixed model: EQI change → disease mortality change,
# stratified by RUCC (Rural-Urban Continuum Code, values 1-4).
# Phase 1: National level (Overall EQI + 5 domains).
# Phase 2: Stratified by RUCC (1=most urban, 4=most rural).
...
utils::globalVariables(c(
  "State_FIPS", "RUCC", "delta_AAMR_lower", "delta_AAMR_upper", "delta_Smoking_Rate",
...
stan_file <- file.path(tempdir(), "delta_rucc.stan")
...
req <- c(
  "COUNTY_FIPS", "State_FIPS", "Outcome", "Lag", "RUCC",
...
build_design <- function(d, cat_col) {
  ...
  if (nrow(d) == 0) {
    warning("build_design: No complete cases after filtering. Original n=", original_n,
            ". Check if ", cat_col, " has unexpected values. Unique values in original: ",
            paste(unique(d[[cat_col]]), collapse = ", "))
    return(NULL)
  }
...
run_models <- function(d, lag_v, outcome, dlabel, outfile, model_prefix = "") {
  domains <- list(
  ...
  for (dom in domains) {
    res <- fit_one(d, dom$col)
    model_name <- if (nchar(model_prefix) > 0) paste0(model_prefix, "_", dom$name) else dom$name
    if (is.null(res)) {
      message("  [Skip/Fail] ", model_name, " Lag=", lag_v)
      next
    }
    eff <- extract_effects(res$fit, res$des$names)
    append_rows(outfile, make_row(outcome, lag_v, model_name, eff, nrow(res$des$df)))
    ...
    if (!is.null(rds)) {
      rl_suffix <- if (nchar(model_prefix) > 0) paste0(model_prefix, "_", dom$name) else paste0("RUCC_", dom$name)
      rl_file <- file.path(rl_dir, sprintf("%s_Lag%d_%s.rds", dlabel, lag_v, rl_suffix))
      saveRDS(rds, rl_file)
      message("       → RL: ", basename(rl_file))
    }

    message("  [OK] ", model_name, " Lag=", lag_v, "  Rhat_max=", round(eff$rhat_max, 3))
  }
}
...
for (outcome in selected) {
  ...
  outfile <- file.path(out_dir, paste0(dlabel, "_RUCC.csv"))
  ...
  for (lag_v in lag_vals) {
    ...
    # Phase 1: National
    message("  [Phase 1] National")
    run_models(lag_dt, lag_v, outcome, dlabel, outfile, model_prefix = "")

    # Phase 2: RUCC-stratified (valid RUCC = 1-4)
    rucc_vals <- sort(unique(lag_dt$RUCC[!is.na(lag_dt$RUCC) & lag_dt$RUCC > 0]))
    message("  [Phase 2] RUCC (", length(rucc_vals), " levels)")
    for (rv in rucc_vals) {
      sub_dt <- lag_dt[!is.na(RUCC) & RUCC == rv]
      prefix <- paste0("RUCC_", rv)
      message("    ", prefix, "  n=", nrow(sub_dt))
      if (nrow(sub_dt) < opt$`min-n`) {
        message("    [Skip]")
        next
      }
      run_models(sub_dt, lag_v, outcome, dlabel, outfile, model_prefix = prefix)
    }
  }
