#!/bin/bash
# Slurm array launcher for the cmdstan interaction interval-censored mixed model pipeline
# One task per cancer type; each task runs all domains and scenarios inside the R runner.
# Usage:
#   bash Code/brms/submit_cmdstan_interaction_array.sh         # auto-discovers cancer types and submits an array
#   # or, advanced: sbatch --array=0-<N-1> Code/brms/submit_cmdstan_interaction_array.sh

#SBATCH --partition=kshctest
#SBATCH --job-name=WDP_cmdstan_interaction
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --mem=48G
#SBATCH --time=1-00:00:00
#SBATCH --output=cmdstan_interaction_%A_%a.out
#SBATCH --error=cmdstan_interaction_%A_%a.err
...
RUNNER="Code/brms/cmdstan_interaction.R"
...
# Run the interaction interval-censored pipeline for this cancer type
Rscript "$RUNNER" \
