...
# A separate merge step combines splits into final {prefix}_sensitivity_EQI.csv / Covar.csv
#
...
if (opt$test) { opt$iter <- min(opt$iter, 800); opt$warmup <- min(opt$warmup, 300)
...
# ---- Output file prefix ----
get_prefix <- function(c) {
  if (c == "G20_G30_G12.2_F01_F03") return("NDD")
  if (c == "C00_C97")               return("Cancer")
  gsub("[^A-Za-z0-9]", "_", c)
}
...
# Split files named by group tag — no concurrent write collision
...
# write_csv for first row, append without header for subsequent rows
write_rows <- function(path, df) {
  if (!file.exists(path)) write_csv(df, path)
  else suppressWarnings(write.table(df, path, sep=",", col.names=FALSE, row.names=FALSE, append=TRUE))
}

# ---- Design matrix builder ----
build_design_sensitivity <- function(d, extra_covars = character(0)) {
  d <- d %>% mutate(EQI_factor = factor(EQI, levels=1:5))
  req <- c("EQI_factor","AAMR_Lower","AAMR_Upper","cens","State_FIPS", extra_covars)
  d <- d[complete.cases(d[, req, drop=FALSE]), ]
...
# ---- EQI extractors ----
extract_quintiles <- function(draw_df, names_vec) {
  out <- list(Q1="0.00", Q2="", Q3="", Q4="", Q5="")
  if (any(grepl("EQI_factor\\.L", names_vec))) return(out)
  for (q in 2:5) {
...
extract_quintile_metrics <- function(draw_df, names_vec, summ_df) {
  out <- list(Q2_p=NA, Q3_p=NA, Q4_p=NA, Q5_p=NA,
              Q2_rhat=NA, Q3_rhat=NA, Q4_rhat=NA, Q5_rhat=NA,
              Q2_ess_bulk=NA, Q3_ess_bulk=NA, Q4_ess_bulk=NA, Q5_ess_bulk=NA,
              Q2_ess_tail=NA, Q3_ess_tail=NA, Q4_ess_tail=NA, Q5_ess_tail=NA)
  if (any(grepl("EQI_factor\\.L", names_vec))) return(out)
  for (q in 2:5) {
    idx <- match(paste0("EQI_factor", q), names_vec)
    if (!is.na(idx)) {
...
extract_covar_cells <- function(draw_df, names_vec, model_covars) {
  cells <- setNames(rep("", length(COVAR_ORIG)), COVAR_ORIG)
  for (z_col in model_covars) {
    orig <- COVAR_ORIG[z_col]
    idx  <- match(z_col, names_vec)
    if (!is.na(idx)) cells[[orig]] <- format_cell(draw_df[[paste0("beta[",idx,"]")]])
  }
  cells
}
...
# Z-score standardize on the full cancer dataset (consistent across all tasks)
for (z_col in names(COVAR_ORIG)) {
  orig <- COVAR_ORIG[z_col]
  mu_  <- mean(cancer_dt[[orig]], na.rm=TRUE)
  sd_  <- sd(cancer_dt[[orig]],   na.rm=TRUE)
  cancer_dt[, (z_col) := (get(orig) - mu_) / ifelse(is.na(sd_) | sd_ == 0, 1, sd_)]
}

# ---- Main loop: scenarios × models in this group ----
...
for (sc in scenario_list) {
  ...
  if (nrow(scen_dt) < opt$`min-n`) {
    message("[Skip] ", sc$key, " n=", nrow(scen_dt)); next
  }
  ...
  for (sm in run_models) {
    ...
    if (nrow(des$df) < opt$`min-n`) {
      message("[Skip] ", sc$key, " ", model_tag, " n=", nrow(des$df)); next
    }

    states_s  <- sort(unique(des$df$State_FIPS))
    state_idx <- match(des$df$State_FIPS, states_s)
    data_list <- list(
      N=nrow(des$df), S=length(states_s), state=state_idx,
      y_lower=des$df$AAMR_Lower, y_upper=des$df$AAMR_Upper, cens=des$df$cens,
      K=ncol(des$X), X=des$X
    )
    init_fn <- local({
      K_ <- data_list$K; S_ <- data_list$S
      function() list(beta=rep(0, K_), z_u=rep(0, S_), sigma=50, sigma_u=10)
    })
    ...
    fit <- try(mod$sample(
      ...
      refresh=0, seed=opt$seed,
      init=rep(list(init_fn()), opt$chains)
    ), silent=TRUE)

    if (inherits(fit, "try-error")) {
      message("[Fail] ", sc$key, " ", model_tag); next
    }
    ...
    # EQI quintile row
    q   <- extract_quintiles(draws, des$names)
    met <- extract_quintile_metrics(draws, des$names, summ)
    eqi_row <- tibble(
    ...
    write_rows(covar_file, covar_row)
