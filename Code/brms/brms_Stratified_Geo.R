...
# Stratification variables: Census_Region, Climate_Zone, Economic_type, Homeownership_tertile
...
option_list <- list(
  ...
  make_option(c("--output-dir"), type = "character", default = "Result/brms_Stratified_Geo"),
...
req <- c(
  ...
  "Smoking_rate", "Physical_Activities_rate", "Obesity_rate",
  "Census_Region", "Climate_Zone", "Economic_type", "Homeownership_tertile"
)
...
strat_vars <- c("Census_Region", "Climate_Zone", "Economic_type", "Homeownership_tertile")
...
for (outcome in selected) {
  ...
  outfile_mrd <- file.path(out_dir, paste0(dlabel, "_Stratified_Geo_MRD.csv"))
  outfile_mrr <- file.path(out_dir, paste0(dlabel, "_Stratified_Geo_MRR.csv"))

  for (sv in strat_vars) {
    strat_vals <- sort(unique(na.omit(dt[[sv]])))
    message("  Stratifying by: ", sv, " (", length(strat_vals), " levels)")

    for (val in strat_vals) {
      strat_tag <- paste0(sv, "_", val)
      strat_dt <- dt[!is.na(get(sv)) & get(sv) == val]
      ...
      for (sc in scenario_list) {
        ...
        aamr_p <- sc$aamr
        lagv <- sc$lag
        scen_dt <- strat_dt[EQI_Period == eqi_p & Time_Period == aamr_p & Outcome == outcome]
