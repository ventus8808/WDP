#!/usr/bin/env Rscript
# Interval-censored mixed model pipeline stratified by cluster-based factors.
# Stratification variables: Cluster_EQI, Cluster_NLCD
...
option_list <- list(
  ...
  make_option(c("--output-dir"), type = "character", default = "Result/brms_Stratified_Cluster"),
...
for (outcome in selected) {
  ...
  outfile_mrd <- file.path(out_dir, paste0(dlabel, "_Stratified_Cluster_MRD.csv"))
  outfile_mrr <- file.path(out_dir, paste0(dlabel, "_Stratified_Cluster_MRR.csv"))
