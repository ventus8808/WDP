#!/bin/bash
# Test script for cmdstan_Cluster_0REF analysis - C00_C97 with k=3
# Usage: bash Code/brms/test_cmdstan_Cluster_0REF_C00_C97_k3.sh

#SBATCH --partition=kshctest
#SBATCH --job-name=WDP_Cluster_0REF_test
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --mem=48G
#SBATCH --time=1-00:00:00
#SBATCH --output=test_cmdstan_Cluster_0REF_C00_C97_k3_%j.out
#SBATCH --error=test_cmdstan_Cluster_0REF_C00_C97_k3_%j.err
...
RUNNER="Code/brms/cmdstan_Cluster_0REF.R"

if [ ! -f "$RUNNER" ]; then
  log ERROR "找不到R脚本: $RUNNER"; exit 1
fi

log INFO "开始测试: cmdstan_Cluster_0REF C00_C97 with k=3"
...
# Run the cmdstan_Cluster_0REF analysis for C00_C97 with k=3
Rscript "$RUNNER" \
...
log INFO "✅ 测试完成: cmdstan_Cluster_0REF C00_C97_k3"
