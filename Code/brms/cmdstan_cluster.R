...
option_list <- list(
  make_option(c("--data"), type="character", default="Data/Processed/df_EQI_AAMR_Triangulation/EQI_AAMR_Cluster_Climate.csv", help="Input interval data with cluster IDs"),
  ...
  for(k_val in k_values){
    ...
    for(sc in scenario_list){
      ...
      if(inherits(fit_overall,"try-error")){ message("[Fail] Overall EQI model ", scen_key); } else {
        ...
        row_over <- tibble(
          ICD_Code=cancer, EQI_Period=eqi_out, AAMR_Period=aamr_out, Lag=lagv, Model=paste0(cluster_title,"_EQI"),
      ...
      if(inherits(fit_multi,"try-error")){ message("[Fail] Multi-domain model ", scen_key) } else {
        ...
        for(dom in domain_prefix){
          ...
          row_dom <- tibble(
            ICD_Code=cancer, EQI_Period=eqi_out, AAMR_Period=aamr_out, Lag=lagv, Model=paste0(cluster_title, "_", sub("_factor","",dom)),
