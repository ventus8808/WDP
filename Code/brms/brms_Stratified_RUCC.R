#!/usr/bin/env Rscript
# Interval-censored mixed model pipeline stratified by RUCC (1-4).
# Within each RUCC stratum, uses RUCC-specific EQI quintiles (RUCC_EQI, RUCC_EQI_Air, ...).
...
option_list <- list(
  ...
  make_option(c("--output-dir"), type = "character", default = "Result/brms_Stratified_RUCC"),
...
req <- c(
  "COUNTY_FIPS", "EQI_Period", "Time_Period", "Lag_Years", "Outcome", "RUCC",
...
# Design matrices use RUCC_EQI* columns (quintiles within each RUCC stratum)
build_design_overall <- function(d) {
  d <- d %>% mutate(EQI_factor = factor(RUCC_EQI, levels = 1:5))
...
build_design_multi <- function(d) {
  d <- d %>% mutate(
    EQI_Air_factor    = factor(RUCC_EQI_Air, levels = 1:5),
    EQI_Water_factor  = factor(RUCC_EQI_Water, levels = 1:5),
    EQI_Land_factor   = factor(RUCC_EQI_Land, levels = 1:5),
    EQI_Built_factor  = factor(RUCC_EQI_Built, levels = 1:5),
    EQI_Social_factor = factor(RUCC_EQI_Social, levels = 1:5)
  )
  d <- d[complete.cases(d[, c(
...
for (outcome in selected) {
  ...
  outfile_mrd <- file.path(out_dir, paste0(dlabel, "_Stratified_RUCC_MRD.csv"))
  outfile_mrr <- file.path(out_dir, paste0(dlabel, "_Stratified_RUCC_MRR.csv"))

  rucc_vals <- sort(unique(na.omit(dt[Outcome == outcome]$RUCC)))
  message("  RUCC strata: ", paste(rucc_vals, collapse = ", "))

  for (rucc_val in rucc_vals) {
    strat_tag <- paste0("RUCC", rucc_val)
    strat_dt <- dt[Outcome == outcome & !is.na(RUCC) & RUCC == rucc_val]
    message("  ", strat_tag, " (n=", nrow(strat_dt), ")")
    ...
    for (sc in scenario_list) {
      ...
      if (nrow(scen_dt) < opt$`min-n`) {
        message("[Skip] ", strat_tag, " ", scen_key, " n=", nrow(scen_dt))
        next
      }
      eqi_out <- gsub("-", "_", eqi_p)
      aamr_out <- gsub("-", "_", aamr_p)

      # Overall RUCC_EQI + SM + PA + OB
      des_overall <- build_design_overall(scen_dt)
      if (nrow(des_overall$df) < opt$`min-n`) {
        message("[Skip] ", strat_tag, " ", scen_key, " EQI n=", nrow(des_overall$df))
      } else {
        ...
        if (inherits(fit_overall, "try-error")) {
          message("[Fail] ", strat_tag, " ", scen_key, " EQI+SM+PA+OB")
        } else {
          ...
          row_over_mrd <- tibble(
            ...
            Model = paste0(strat_tag, "_EQI+SM+PA+OB"),
          ...
          message("[OK] ", strat_tag, " ", scen_key, " EQI+SM+PA+OB")
        }
      }

      # Multi-domain RUCC_EQI sub-indices + SM + PA + OB
      des_multi <- build_design_multi(scen_dt)
      if (nrow(des_multi$df) < opt$`min-n`) {
        message("[Skip] ", strat_tag, " ", scen_key, " Multi-domain n=", nrow(des_multi$df))
      } else {
        ...
        if (inherits(fit_multi, "try-error")) {
          message("[Fail] ", strat_tag, " ", scen_key, " Multi-domain")
        } else {
          ...
          for (dom in domain_prefix) {
            ...
            row_dom_mrd <- tibble(
              ICD_Code = outcome, EQI_Period = eqi_out, AAMR_Period = aamr_out, Lag = lagv,
              Model = paste0(strat_tag, "_", sub("_factor", "", dom), "+SM+PA+OB"),
          ...
          message("[OK] ", strat_tag, " ", scen_key, " Multi-domain+SM+PA+OB")
        }
      }
    }
  }
