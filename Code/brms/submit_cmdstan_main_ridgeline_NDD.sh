#!/bin/bash
# Slurm array launcher for ridgeline posterior extraction (NDD: G20_G30_G12.2_F01_F03)
# Each task runs one lag scenario (5, 10, or 15 years) with both Overall and Multi-domain models
# Usage:
#   sbatch --array=1-3 Code/brms/submit_cmdstan_main_ridgeline_NDD.sh

#SBATCH --partition=kshctest
#SBATCH --job-name=WDP_ridgeline_NDD
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --mem=48G
#SBATCH --time=1-00:00:00
#SBATCH --output=ridgeline_NDD_%A_%a.out
#SBATCH --error=ridgeline_NDD_%A_%a.err
...
log INFO "开始处理任务 $TASK_ID: $DESC (NDD)"
...
# --- Run Overall model ---
log INFO "运行 Overall EQI 模型 (Scenario=$SCENARIO, NDD)"
Rscript "$RUNNER" \
  --scenario "$SCENARIO" \
  --model "overall" \
  --cancer "$CANCER" \
...
if [ $? -ne 0 ]; then
  log ERROR "Overall 模型失败 (Scenario=$SCENARIO, NDD)"; exit 1
fi
log INFO "✓ Overall 模型完成"

# --- Run Multi-domain model ---
log INFO "运行 Multi-domain 模型 (Scenario=$SCENARIO, NDD)"
Rscript "$RUNNER" \
  --scenario "$SCENARIO" \
  --model "multi" \
  --cancer "$CANCER" \
...
if [ $? -ne 0 ]; then
  log ERROR "Multi-domain 模型失败 (Scenario=$SCENARIO, NDD)"; exit 1
fi
log INFO "✓ Multi-domain 模型完成"

log INFO "✅ 完成任务 $TASK_ID: $DESC (NDD - Overall + MultiDomain)"
