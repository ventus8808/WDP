...
class LMMMIDataPreprocessor:
    ...
    def create_comprehensive_dataset(self) -> pd.DataFrame:
        ...
        if all_datasets:
            ...
            # 删除缺少EQI和SR的记录
            logger.info("=== 删除缺少EQI和SR的记录 ===")
            initial_shape = combined_data.shape
            combined_data = combined_data.dropna(subset=['EQI', 'SR'], how='any')
            logger.info(f"删除缺失记录后: {initial_shape} -> {combined_data.shape}")
    ...
    def _clean_and_convert_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        eqi_columns = ['RUCC', 'EQI', 'EQI_Air', 'EQI_Water', 'EQI_Land', 'EQI_Built', 
                       'EQI_Social', 'RUCC_EQI', 'RUCC_EQI_Air', 'RUCC_EQI_Water', 
                       'RUCC_EQI_Land', 'RUCC_EQI_Built', 'RUCC_EQI_Social']
    ...
    def analyze_missing_patterns(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        for var in df.columns:
            missing_pct = missing_props[var] * 100
            print(f"{var}: {missing_pct:.2f}%")
        
        # 关键变量完整案例统计
        total_complete_cases = df.dropna(subset=key_variables).shape[0]
        total_cases = len(df)
        overall_missing_rate = 1 - total_complete_cases/total_cases
        
        print(f"\n=== 关键变量完整性统计 ===")
        print(f"总案例数: {total_cases}")
        print(f"完整案例数: {total_complete_cases}")
        print(f"总体缺失比例: {overall_missing_rate:.3f}")
        
        logger.info(f"完整案例数: {total_complete_cases}")
        logger.info(f"总案例数: {total_cases}")
        logger.info(f"缺失比例: {overall_missing_rate:.3f}")
        
        return {
            'total_complete_cases': total_complete_cases,
