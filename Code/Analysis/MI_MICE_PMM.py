...
class MICEImputerWithPMM:
    ...
    def mice_with_pmm_single_imputation(self, df: pd.DataFrame, var_types: Dict[str, List[str]], 
        ...
        for iteration in range(self.max_iter):
            for var in missing_vars:
                ...
                # 准备预测变量（只使用数值变量，但排除Smoking_Rate）
                exclude_vars = ['COUNTY_FIPS', 'State', 'EQI_Period', 'AAMR_Period', 'HHS_Region', 'Census_Region', 'Census_Division', 'Smoking_Rate', 'SR']
    ...
    def convert_to_long_format(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        将宽格式数据转换为长格式（类似EQI_AAMR_Point.csv的格式）
        ...
        base_cols = ['COUNTY_FIPS', 'State', 'RUCC', 'EQI', 'EQI_Air', 'EQI_Water', 
                    'EQI_Land', 'EQI_Built', 'EQI_Social', 'RUCC_EQI', 
                    'RUCC_EQI_Air', 'RUCC_EQI_Water', 'RUCC_EQI_Land', 'RUCC_EQI_Built', 
                    'RUCC_EQI_Social', 'SR', 'EQI_Period', 'AAMR_Period']
        ...
        for _, row in df.iterrows():
            ...
            for aamr_col in aamr_vars:
                if aamr_col in cancer_mapping and pd.notna(row[aamr_col]):
                    ...
                    # 确保值是字符串后再进行操作
                    eqi_period = str(row.get('EQI_Period', '')) if pd.notna(row.get('EQI_Period')) else ''
                    aamr_period = str(row.get('AAMR_Period', '')) if pd.notna(row.get('AAMR_Period')) else ''
                    
                    # 根据EQI_Period和AAMR_Period确定Time_Period和Lag_Years
                    time_period_map = {
                        ('2000_2005', '2006_2010'): ('2006-2010', 5),
                        ('2000_2005', '2011_2015'): ('2011-2015', 10),
                        ('2006_2010', '2011_2015'): ('2011-2015', 5),
                        ('2006_2010', '2016_2020'): ('2016-2020', 10)
                    }
                    
                    time_period, lag_years = time_period_map.get(
                        (eqi_period, aamr_period), 
                        ('Unknown', 0)
                    )
                    
                    # 确保 COUNTY_FIPS 是字符串
                    county_fips = str(row.get('COUNTY_FIPS', '')) if pd.notna(row.get('COUNTY_FIPS')) else ''
                    
                    record.update({
                        'Smoking_Rate': row.get('SR', row.get('Smoking_Rate')),
                        'Time_Period': time_period,
                        'Lag_Years': lag_years,
                        'EQI_Period': eqi_period.replace('_', '') if '_' in eqi_period else eqi_period,
                        'AAMR': row[aamr_col],
                        'Cancer_Type': cancer_type,
                        'Cancer_Description': cancer_desc,
                        'State_FIPS': county_fips[:2] if len(county_fips) >= 2 else ''  # 前两位是州FIPS
                    })
        ...
        # 重新排列列顺序以匹配EQI_AAMR_Point.csv
        target_cols = ['COUNTY_FIPS', 'State', 'Time_Period', 'Lag_Years', 'EQI_Period', 
                      'Cancer_Type', 'AAMR', 'Smoking_Rate', 'RUCC', 'EQI', 'EQI_Air', 'EQI_Water', 
                      'EQI_Land', 'EQI_Built', 'EQI_Social', 'RUCC_EQI', 'RUCC_EQI_Air', 
                      'RUCC_EQI_Water', 'RUCC_EQI_Land', 'RUCC_EQI_Built', 'RUCC_EQI_Social']
    ...
    def save_final_dataset(self, final_df: pd.DataFrame, long_df: pd.DataFrame) -> Dict[str, str]:
        ...
        # 只保存点估计格式（与EQI_AAMR_Point.csv格式相同）
        point_filepath = self.project_root / "Data" / "Processed" / "df_EQI_AAMR" / "EQI_AAMR_Point_MICE.csv"
        point_filepath.parent.mkdir(parents=True, exist_ok=True)
        long_df.to_csv(point_filepath, index=False)
        saved_files['point_format'] = str(point_filepath)
        
        logger.info(f"点估计格式数据已保存: {point_filepath}")
    ...
    def run_simple_imputation_with_diagnostics(self) -> Dict[str, Any]:
        ...
        # 保存结果摘要
        diagnostics_dir = self.project_root / "Result" / "EQI_LMM_MI_Diagnose"
