...
def plot_aamr_map(time_period, icd, config):
    ...
    aamr_file = "Data/Processed/df_EQI_AAMR/EQI_AAMR_Point_MICE.csv"
    ...
    # Legend with ranges
    legend_elements = [
        mpatches.Patch(color=AAMR_COLORS[i + 1], label=bin_labels[i]) for i in range(7)
    ]
    ax.legend(
    ...
    # Title
    cancer_name = config["brms_analysis"]["icd_mapping"].get(icd, icd)
    plt.suptitle(
        f"Age-adjusted Mortality Rate Distribution Map\nof {cancer_name} ({time_period})",
        y=0.82,
        ha="center",
    )

    # Save
    output_filename = os.path.join(output_dir, f"Map_AAMR_{time_period}_{icd}.png")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Plot Age-Adjusted Mortality Rate (AAMR) maps for specified ICD-10 cancer codes.\n\n"
        "This script generates choropleth maps of AAMR data across US counties, using custom percentiles "
        "and a diverging color scheme. Maps are saved in the Result/EQI_Map/ directory.\n\n"
        "Examples:\n"
        "  python Map_AAMR.py --icd C00_C97  # Plot for All-site Cancer\n"
        "  python Map_AAMR.py --icd C34      # Plot for Lung Cancer\n"
        "  python Map_AAMR.py --icd all      # Plot for all available ICD codes"
    )
    parser.add_argument(
        "--icd",
        default="C00_C97",
        help='ICD code to plot (e.g., C00_C97, C34). Use "all" to plot for all available ICD codes.',
    )
    ...
    # Determine ICD codes to plot
    if args.icd == "all":
        icds = df_aamr["Cancer_Type"].unique()
    else:
        icds = [args.icd]
    ...
    for icd in icds:
        ...
        if df_filtered.empty:
            print(f"No data for ICD {icd}")
            continue
        unique_time_periods = df_filtered["Time_Period"].unique()

        # Plot for each unique time period for this ICD
